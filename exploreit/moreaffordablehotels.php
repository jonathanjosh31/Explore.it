...
if(isset($_GET['hotel_name'])){
    $hotelname = mysqli_real_escape_string($configconn, $_GET['hotel_name']);
    
    $sql = "SELECT * FROM affordable_hotels";

    $result = mysqli_query($configconn , $sql);
    $the_hotel = array('hotel_name' => '' ,'description' => '' , 'address' => '','imglink' =>'' , 'created_at' => '');
    $hotel = mysqli_fetch_all($result,MYSQLI_ASSOC); 
    for($i=0;$i<count($hotel);$i++)
    {
        if($hotel[$i]['hotel_name']==$hotelname)
        {
            $the_hotel['hotel_name']=$hotel[$i]['hotel_name'];
            ...
            <div class="tm-page-content-width">
                <div class="tm-bg-black-translucent tm-content-box tm-content-box-right tm-flex-center">
                    <div class="tm-content-box-inner">
                        <h2 class="tm-section-title"><?php  echo $the_hotel['hotel_name']; ?></h2>
