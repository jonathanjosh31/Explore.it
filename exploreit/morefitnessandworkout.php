...
if(isset($_GET['center_name'])){
    $hotelname = mysqli_real_escape_string($configconn, $_GET['center_name']);
    
    $sql = "SELECT * FROM fitnessandworkout";

    $result = mysqli_query($configconn , $sql);
    $the_hotel = array('center_name' => '' ,'description' => '' , 'address' => '','imglink' =>'' , 'created_at' => '');
    $hotel = mysqli_fetch_all($result,MYSQLI_ASSOC); 
    for($i=0;$i<count($hotel);$i++)
    {
        if($hotel[$i]['center_name']==$hotelname)
        {
            $the_hotel['center_name']=$hotel[$i]['center_name'];
            ...
            <div class="tm-page-content-width">
                <div class="tm-bg-black-translucent tm-content-box tm-content-box-right tm-flex-center">
                    <div class="tm-content-box-inner">
                        <h2 class="tm-section-title"><?php  echo $the_hotel['center_name']; ?></h2>
