...
if(isset($_GET['place_name'])){
    $hotelname = mysqli_real_escape_string($configconn, $_GET['place_name']);
    
    $sql = "SELECT * FROM creativeandfun";

    $result = mysqli_query($configconn , $sql);
    $the_hotel = array('place_name' => '' ,'description' => '' , 'address' => '','imglink' =>'' , 'created_at' => '');
    $hotel = mysqli_fetch_all($result,MYSQLI_ASSOC); 
    for($i=0;$i<count($hotel);$i++)
    {
        if($hotel[$i]['place_name']==$hotelname)
        {
            $the_hotel['place_name']=$hotel[$i]['place_name'];
            ...
            <div class="tm-page-content-width">
                <div class="tm-bg-black-translucent tm-content-box tm-content-box-right tm-flex-center">
                    <div class="tm-content-box-inner">
                        <h2 class="tm-section-title"><?php  echo $the_hotel['place_name']; ?></h2>
