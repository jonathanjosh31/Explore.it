...
if(isset($_GET['institution_name'])){
    $hotelname = mysqli_real_escape_string($configconn, $_GET['institution_name']);
    
    $sql = "SELECT * FROM outofthebox";

    $result = mysqli_query($configconn , $sql);
    $the_hotel = array('institution_name' => '' ,'description' => '' , 'address' => '','imglink' =>'' , 'created_at' => '');
    $hotel = mysqli_fetch_all($result,MYSQLI_ASSOC); 
    for($i=0;$i<count($hotel);$i++)
    {
        if($hotel[$i]['institution_name']==$hotelname)
        {
            $the_hotel['institution_name']=$hotel[$i]['institution_name'];
            ...
            <div class="tm-page-content-width">
                <div class="tm-bg-black-translucent tm-content-box tm-content-box-right tm-flex-center">
                    <div class="tm-content-box-inner">
                        <h2 class="tm-section-title"><?php  echo $the_hotel['institution_name']; ?></h2>
