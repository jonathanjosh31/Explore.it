...
if(isset($_GET['shop_name'])){
    $hotelname = mysqli_real_escape_string($configconn, $_GET['shop_name']);
    
    $sql = "SELECT * FROM gettingdigital";

    $result = mysqli_query($configconn , $sql);
    $the_hotel = array('shop_name' => '' ,'description' => '' , 'address' => '','imglink' =>'' , 'created_at' => '');
    $hotel = mysqli_fetch_all($result,MYSQLI_ASSOC); 
    for($i=0;$i<count($hotel);$i++)
    {
        if($hotel[$i]['shop_name']==$hotelname)
        {
            $the_hotel['shop_name']=$hotel[$i]['shop_name'];
            ...
            <div class="tm-page-content-width">
                <div class="tm-bg-black-translucent tm-content-box tm-content-box-right tm-flex-center">
                    <div class="tm-content-box-inner">
                        <h2 class="tm-section-title"><?php  echo $the_hotel['shop_name']; ?></h2>
